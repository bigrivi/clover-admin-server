...
var treeBase = function(baseModel){
	var onPostBefore= function(req, res, next)
	{
		var parentId = req.body.parentId
		if(parentId){
			baseModel.findById(parentId,function(err,parentInfo){
				async.series([
					function(callback){
						//更新右值 start
						...
						baseModel.update(conditions, update, options, function(err,res){
							if(err){
								console.log(err);
							}
							if(res){
	...
	{ 
		...
		baseModel.findById(id,function(err,nodeInfo){
			if(nodeInfo["lft"]==1){
			...
			async.series([
					function(callback){
						...
						baseModel.remove(conditions, function(err,res){
							if(err){
								console.log(err);
							}
							if(res){
	...
	var getTreeData = function(parentId,includeParent,populates,callback)
	{
		var findParams = {}
			...
			baseModel.findOne(findParams).sort({ _id: 1 }).exec(function(err, data) {
				parentData = data;
				if(data==null){
					callback([])
					return;
				...
				if(populates){
					baseModel.find({lft: { $gte: parentData["lft"], $lte: parentData["rgt"] }}).populate(populates).sort({ lft: 1 }).exec(function(err, list) {
						callback(list)
					});
				}
				else{
					baseModel.find({lft: { $gte: parentData["lft"], $lte: parentData["rgt"] }}).sort({ lft: 1 }).exec(function(err, list) {
						callback(list)
					});
				}
...
exports = module.exports = treeBase
